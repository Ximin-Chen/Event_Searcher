...
def login():
    ...
    if request.method == 'POST':
        data2 = request.json
        user = User.query.filter_by(username=data2.get('user_id')).first()
        print(user)
        if user is not None:
            print(user.password_hash)
            print(data2.get('password'))
            if user.password_hash == data2.get('password'):
                session['user_id'] = data2.get('user_id')
                return jsonify(status='OK', name='olivia')
...
def register():
    ...
    if user is not None:
        return jsonify(status='error')
    user = User(username=data.get('user_id'), password_hash=data.get('password'))
